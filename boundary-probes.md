...
| **Done** (feature built or already exists) | 13 | #67 (confidence badges), #75 (lawyer recommendations → counter-draft), #78 (plain English rewrite → counter-draft), #85 (ARIA labels), #87 (different prompts → now on-demand depth buttons), #88 (share analysis → HTML export), #91 (risk timeline → `/timeline/`), #93 (offline export → self-contained HTML), #95 (negotiate-for-me → counter-draft), #96 (URL submission), #98 (what's missing → verdict+on-demand) |
