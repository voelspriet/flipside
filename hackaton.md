...
| 53 | Upload a 100-page document and measure where analysis quality drops — find the context window boundary for clause-by-clause accuracy | **PASSED — 222 pages (~167K tokens), 36/36 planted traps caught across 9 documents, no degradation found. Bottleneck is Haiku output budget not Opus reasoning. See Entry 76.** | Scale |
...
| 58 | Upload a document with hidden Unicode characters or zero-width spaces between words — does parsing break? | **partially addressed → garbled/reversed text cleanup via Haiku gate** | Adversarial |
...
| 62 | Upload a document mixing 3+ languages within the same clauses (e.g., Swiss trilingual contract) — does the analysis switch languages per clause? | **partially addressed → bilingual card translations** | Multilingual |
...
| 72 | Submit a document in right-to-left script (Arabic/Hebrew contract) — does the UI render correctly? Does analysis quality hold? | **partially addressed → bilingual EN translations for non-English docs** | Multilingual |
...
| 82 | Add keyboard-only navigation for the entire analysis flow — test full accessibility without a mouse | **partially addressed → arrow keys + 1-9 card nav + Space/Enter flip** | Accessibility |
...
| 94 | Upload a document that changes language mid-sentence (code-switching, common in some jurisdictions) — does analysis stay coherent? | **partially addressed → bilingual analysis maintains doc language + EN translations** | Multilingual |
